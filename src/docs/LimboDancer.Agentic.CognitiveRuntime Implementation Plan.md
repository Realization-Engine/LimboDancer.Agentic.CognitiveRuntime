...
The [ASL-OT-02 TIR Schema and Deterministic Extraction Design](<../../docs/ASL/LimboDancer.Agentic.CognitiveRuntime ASL-OT-02 TIR Schema and Deterministic Extraction Design.md>) now governs the next eligible slice. ASL-OT-02 extends the isolated C#/.NET ASL authoring solution outside the runtime kernel and will generate deterministic structural TIR artifacts and diagnostics. ASL-OT-01 registry generation and fragment location have been ported to that solution with committed-manifest and representative-case parity tests. `utils/pdf_to_markdown.py` remains an outside conversion utility only. Ontology semantics, publication, runtime resolution, and provider evidence remain deferred.
