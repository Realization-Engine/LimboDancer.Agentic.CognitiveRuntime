...
The [ASL-OT-02 TIR Schema and Deterministic Extraction Design](<../../docs/ASL/LimboDancer.Agentic.CognitiveRuntime ASL-OT-02 TIR Schema and Deterministic Extraction Design.md>) governs the active slice. Its C# foundation defines the versioned TIR schema, ASL-owned envelope and structural artifact types, deterministic artifact identity, canonical JSON serialization, and fail-closed authority checks. The structural extractor maps the established ASL-OT-01 fragments into `SourceFragment`, `Section`, and `Rule` artifacts, preserves ordered evidence, and derives evidence-qualified hierarchy results. TIR 1.3 represents explicit major-section declarations as typed structural anchors, keeps general Rule `A.1` distinct from Section `A1`, follows ASL digit nesting (`A1.1` → `A1`, `A1.11` → `A1.1`), and recognizes the zero-padded child convention (`A7.301` → `A7.3`, `A14.01` → `A14`) only when the exact parent exists. Source-fragment references now distinguish whole-fragment evidence from exact, portable UTF-8 byte spans; cross-reference tokens, `EX:` markers, and structured-text Section declarations are the first exact-span consumers. Bare numbered notes and list items are excluded from Rule boundaries by a tested source convention. All extracted artifacts remain unmodeled and unaccepted, while eight damaged parent boundaries and 46 missing reference targets remain diagnostics. A committed metadata-only sample is checked byte-for-byte against C# regeneration. The next increment may recover `A7.37`, `A7.8`, and `E1.93` through unique sub-fragment markers before defining complete example spans or table rows. `utils/pdf_to_markdown.py` remains an outside conversion utility only. Ontology semantics, publication, runtime resolution, and provider evidence remain deferred.
