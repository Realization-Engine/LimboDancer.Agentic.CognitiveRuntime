...
ASL-OT-01 is complete and approved in the [ASL-OT-01 Source Registry Review](<../../docs/ASL/LimboDancer.Agentic.CognitiveRuntime ASL-OT-01 Source Registry Review.md>). It registers seven Markdown artifacts and 661 images, pins source and converter hashes, supplies deterministic structural fragment locators, and retains an explicitly unverified fourteen-fragment review sample.

The [ASL-OT-02 TIR Schema and Deterministic Extraction Design](<../../docs/ASL/LimboDancer.Agentic.CognitiveRuntime ASL-OT-02 TIR Schema and Deterministic Extraction Design.md>) governs the active slice. Its C# foundation defines the versioned TIR schema, ASL-owned envelope and structural artifact types, deterministic artifact identity, canonical JSON serialization, and fail-closed authority checks. The structural extractor maps the established ASL-OT-01 fragments into `SourceFragment`, `Section`, and `Rule` artifacts, preserves ordered evidence, and derives evidence-qualified hierarchy results. TIR 1.3 represents explicit major-section declarations as typed structural anchors, keeps general Rule `A.1` distinct from Section `A1`, follows ASL digit nesting (`A1.1` → `A1`, `A1.11` → `A1.1`), and recognizes the zero-padded child convention (`A7.301` → `A7.3`, `A14.01` → `A14`) only when the exact parent exists. Source-fragment references distinguish whole-fragment evidence from exact, portable UTF-8 byte spans. Extractor 1.5 uses those spans to recover the required `A7.37`, `A7.8`, and `E1.93` boundaries from unique explicit embedded markers without changing the converted source or inferring semantics. This raises the corpus to 2,001 Rules, resolves all eight missing-parent diagnostics, and reduces missing reference targets from 46 to 32. Unneeded marker lookalikes are ignored, while duplicate eligible markers remain ambiguous and diagnostic. All extracted artifacts remain unmodeled and unaccepted. A committed metadata-only sample is checked byte-for-byte against C# regeneration. Complete example extents and table-row reconstruction remain deferred. `utils/pdf_to_markdown.py` remains an outside conversion utility only. Ontology semantics, publication, runtime resolution, and provider evidence remain deferred.
