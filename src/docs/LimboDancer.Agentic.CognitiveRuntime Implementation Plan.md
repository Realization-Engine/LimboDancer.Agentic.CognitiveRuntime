...
The [ASL-OT-02 TIR Schema and Deterministic Extraction Design](<../../docs/ASL/LimboDancer.Agentic.CognitiveRuntime ASL-OT-02 TIR Schema and Deterministic Extraction Design.md>) governs the active slice. Its C# foundation defines the versioned TIR schema, ASL-owned envelope and structural artifact types, deterministic artifact identity, canonical JSON serialization, and fail-closed authority checks. The structural extractor maps the established ASL-OT-01 fragments into `SourceFragment` and `Rule` artifacts, preserves ordered evidence, and derives evidence-qualified hierarchy results. It now also captures only explicit chapter-qualified cross-references, uppercase `EX:` markers, and labelled fenced table/chart blocks. Bare numbered notes and list items are excluded from Rule boundaries by a tested source convention. All extracted artifacts remain unmodeled and unaccepted, while missing parents and reference targets remain diagnostics. A committed metadata-only sample is checked byte-for-byte against C# regeneration. The next increment must decide how section headings participate in hierarchy before reducing missing-parent diagnostics, and should keep exact example-span and table-row recovery separate from semantic formalization. `utils/pdf_to_markdown.py` remains an outside conversion utility only. Ontology semantics, publication, runtime resolution, and provider evidence remain deferred.
