...
The [ASL-OT-02 TIR Schema and Deterministic Extraction Design](<../../docs/ASL/LimboDancer.Agentic.CognitiveRuntime ASL-OT-02 TIR Schema and Deterministic Extraction Design.md>) governs the active slice. Its first C# foundation now defines the versioned TIR schema, ASL-owned envelope and structural artifact types, deterministic artifact identity, canonical JSON serialization, and fail-closed authority checks. The next increment maps the established ASL-OT-01 fragments into structural `SourceFragment` and `Rule` artifacts with hierarchy candidates and deterministic diagnostics. `utils/pdf_to_markdown.py` remains an outside conversion utility only. Ontology semantics, publication, runtime resolution, and provider evidence remain deferred.
