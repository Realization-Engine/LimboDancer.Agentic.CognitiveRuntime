...
The [ASL-OT-02 TIR Schema and Deterministic Extraction Design](<../../docs/ASL/LimboDancer.Agentic.CognitiveRuntime ASL-OT-02 TIR Schema and Deterministic Extraction Design.md>) governs the active slice. Its C# foundation defines the versioned TIR schema, ASL-owned envelope and structural artifact types, deterministic artifact identity, canonical JSON serialization, and fail-closed authority checks. The structural extractor now maps the established ASL-OT-01 fragments into `SourceFragment` and `Rule` artifacts, preserves ordered evidence, derives evidence-qualified hierarchy results, and emits deterministic duplicate, missing-parent, and ambiguous-parent diagnostics. A committed metadata-only sample is checked byte-for-byte against C# regeneration. The next increment captures explicit cross-reference occurrences and improves deterministic classification of repeated rule identifiers, examples, and table boundaries. `utils/pdf_to_markdown.py` remains an outside conversion utility only. Ontology semantics, publication, runtime resolution, and provider evidence remain deferred.
