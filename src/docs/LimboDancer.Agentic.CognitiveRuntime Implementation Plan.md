...
The [ASL-OT-02 TIR Schema and Deterministic Extraction Design](<../../docs/ASL/LimboDancer.Agentic.CognitiveRuntime ASL-OT-02 TIR Schema and Deterministic Extraction Design.md>) governs the active slice. Its C# foundation defines the versioned TIR schema, ASL-owned envelope and structural artifact types, deterministic artifact identity, canonical JSON serialization, and fail-closed authority checks. The structural extractor maps the established ASL-OT-01 fragments into `SourceFragment`, `Section`, and `Rule` artifacts, preserves ordered evidence, and derives evidence-qualified hierarchy results. TIR 1.2 represents explicit major-section declarations as typed structural anchors, keeps general Rule `A.1` distinct from Section `A1`, follows ASL digit nesting (`A1.1` → `A1`, `A1.11` → `A1.1`), and recognizes the zero-padded child convention (`A7.301` → `A7.3`, `A14.01` → `A14`) only when the exact parent exists. It also captures only explicit chapter-qualified cross-references, uppercase `EX:` markers, and labelled fenced table/chart blocks. Bare numbered notes and list items are excluded from Rule boundaries by a tested source convention. All extracted artifacts remain unmodeled and unaccepted, while missing parents and reference targets remain diagnostics. The completed audit reduces missing-parent diagnostics from 67 to eight without suppressing damaged-boundary findings; 46 missing reference targets remain. A committed metadata-only sample is checked byte-for-byte against C# regeneration. The next increment should establish sub-fragment source spans before recovering damaged rule boundaries, exact example spans, or table rows. `utils/pdf_to_markdown.py` remains an outside conversion utility only. Ontology semantics, publication, runtime resolution, and provider evidence remain deferred.
