...
Corpus preparation and execution are governed by the [Decision Evaluation Corpus Specification and Runbook](<./LimboDancer.Agentic.CognitiveRuntime Decision Evaluation Corpus Specification and Runbook.md>). The ASL 3.01 rulebook Markdown feeds a reviewed, versioned ASL ontology/rule package that may supply semantic evidence and expert case-construction inputs under that process, but neither the source nor its ontology projection is historical Decision evidence or a provider-adoption corpus by itself.
