...
- `ASL Unit Random Selection and Declined OVR Design.md`: step 9 of the unit sequence (proposed): system dice in the game store, Random Selection among several concealed units before the PostReveal forced back, open attempts as state, and the pending OVR declaration whose decline commits the reviewed delegation.
