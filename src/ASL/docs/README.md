...
- `LimboDancer.Agentic.CognitiveRuntime ASL Unit Requirements.md`: proposed requirements (ASL-UNIT-001 onward) for the unit model, its package boundary, and the unit display. Section 13 sets the sequence; step 8 (occupied and concealed entry in live play) is built, step 9 (Random Selection and the declined OVR) is built, step 10 (the OVR NTC review) is done, step 11 (the Infantry OVR in live play) is built, and step 12 (composed maps in live play) is next.
...
- `ASL Unit Composed Maps Design.md`: step 12 of the unit sequence (proposed): placed and reversed boards in the game record and setup, a geometry-only map layout and a composed read with neighbours across a seam, entry facts across a seam with the Scenario A1 terrain evidence unchanged, and the game's map on the Play page.
