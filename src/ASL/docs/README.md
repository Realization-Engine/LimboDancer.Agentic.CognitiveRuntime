...
- `LimboDancer.Agentic.CognitiveRuntime ASL Unit Requirements.md`: proposed requirements (ASL-UNIT-001 onward) for the unit model, its package boundary, and the unit display. Section 13 sets the sequence; step 8 (occupied and concealed entry in live play) is built, step 9 (Random Selection and the declined OVR) is built, step 10 (the OVR NTC review) is next, and step 11 (the Infantry OVR in live play) is outlined.
...
- `Scenario A1 OVR NTC Review 2026-09-26.md`: step 10 of the unit sequence: the sources and the user's rulings for an elected Infantry OVR after a lone concealed SMC reveal (the second reveal on election by Random Selection, the NTC resolution, and a failed NTC), and the case matrix they give.
