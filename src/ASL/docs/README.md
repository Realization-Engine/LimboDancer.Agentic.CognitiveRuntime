...
- `LimboDancer.Agentic.CognitiveRuntime ASL Unit Requirements.md`: proposed requirements (ASL-UNIT-001 onward) for the unit model, its package boundary, and the unit display. Section 13 sets the sequence; step 8 (occupied and concealed entry in live play) is built, step 9 (Random Selection and the declined OVR) is built, step 10 (the OVR NTC review) is done, step 11 (the Infantry OVR in live play) is built, step 12 (composed maps in live play) is built, and step 13 (LOS, blocked or clear) is next.
