...
- `Scenario A1 Fire Review 2026-09-26.md`: step 17 of the unit sequence: the IFT and Terrain Chart supplement, the Fire rule fragments, the user's rulings (Cowering, Hindrance, leader loss, ELR, pins, wounds, the Russian HS, concealment), and the case matrix and package they give.
...
- `ASL Unit LOS Slice 3 Design.md`: step 15 of the unit sequence (built): bridges, rowhouse and factory walls, factories, roofless buildings, rubble, bocage, hillocks, railroad embankments, and slopes in the LOS read, with oracle fixtures for boards chosen for them.
- `ASL Unit LOS Result Design.md`: step 16 of the unit sequence (built): the hindrance breakdown, hexside locations with both aiming points, the oracle fixtures for them, and LOS from a unit on the Play page.
