...
**Status:** Bounded supplement registered separately and reviewed through user-delegated xUnit assertions; original registry and its 11 verified subjects unchanged
...
The [bounded chart candidate](<../../../docs/ASL/SourceRegistry/asl-scenario-a1.backmatter-chart-candidate.json>) pins the supplied PDF SHA-256, physical page 698, the SHA-256 of `pdftotext 24.02.0 -f 698 -l 698 -layout` output, the MF header-line hash, and individual extracted-line hashes for the two building rows. The [supplementary registry](<../../../docs/ASL/SourceRegistry/asl-scenario-a1.supplementary-source-registry.json>) binds those locators to the immutable original registry, source commit and hash of a [bounded Markdown transcription](<../../../docs/ASL/SourceRegistry/Supplements/b-terrain-chart-building-entry.md>). It does not import or republish the full PDF page. A [PDF comparison record](<../../../docs/ASL/SourceRegistry/asl-scenario-a1.backmatter-chart-pdf-comparison.json>) confirms the Infantry column, both 2-MF building entries, the adjacent printed note and COT legend against physical page 698; it retains the question of note applicability. This tool-assisted comparison is **not** source-provider fidelity attestation or domain admission.

The source provider explicitly delegated both bounded source-fidelity review and the first-case chart interpretation to executable xUnit assertions. The [decision artifact](<../../../docs/ASL/SourceRegistry/asl-scenario-a1.chart-review-decision.json>) records that authority and pins the separate supplement and PDF comparison by SHA-256. The test asserts the two 2-MF Infantry cells, the stone row's road/VBM note, the chart's controlling role under A4.13 and B23.4 for the declared ordinary-building entry, and rejection of changed evidence. The note is treated as printed with the stone row; the declared case excludes road and VBM. This decision is scoped to this chart subset and case, and does not attest the ten other unverified first-case rule and figure fragments. The supplement registry remains an immutable *pre-review* registration artifact with its original unverified status.
...
For Scenario A1, A4.13's citation and the two matching 2-MF rows identify the **B. Terrain Chart building rows, applicable row notes and legend** as a companion artifact. The C# assessor's reviewed-chart path removes the source-boundary blocker after validating the delegated decision against the pinned evidence, but still blocks a definitive outcome pending the other rule-source and semantic dependency decisions. The Advanced Sequence of Play (pages 678–681) is a separate timing candidate; an already established MPh fact does not by itself prove that every timing aid is controlling. The concealed-unit chart on page 693 pertains to branches excluded by the declared known-empty case. For stacking, A5.5 is within the already registered Chapter A prose; determine whether it becomes controlling only when `IsBelowStackingLimit` is derived from raw unit state instead of supplied as a declared input fact.

## Fire chart supplement (unit step 17, 2026-09-26)

Unit step 17 adds a second bounded supplement under the boundary rule above: the A7 Infantry Fire Table (physical page 692) and the TEM rows of the B. Terrain Chart (page 698), which A7.3 and A7.6 cite. The [supplement registry](<../../../docs/ASL/SourceRegistry/asl-scenario-a1.fire-chart-supplement.json>) pins the PDF digest, the extractor (`pdftotext 4.00 -table`, whose page 692 rows come out whole, unlike `-layout`), each page-text hash, the two transcription hashes, and a hash for every normalized row, note, and legend item. The earlier page 698 record used poppler `pdftotext 24.02.0 -layout`, so its hashes differ from these; both stay pinned to their own extractor.

The IFT transcription holds the Personnel result cell of every DR row and FP column. The Terrain Chart transcription holds the LOS, TEM, and Notes cells of Open Ground, Brush, Woods, Orchard, Grain, and the wooden and stone building rows, and five legend entries. The Vehicle line, the flame, rubble, and minefield marks, colors, and underlines are outside the supplement.

`AslScenarioA1FireChartSupplement` rebuilds each item from the transcription and requires its hash to equal the one pinned from the extraction, so a changed cell fails even when the transcription digest is re-pinned. The user spot-checked nine cells against rendered images of both pages on 2026-09-26, and the [review decision](<../../../docs/ASL/SourceRegistry/asl-scenario-a1.fire-chart-review-decision.json>) records that delegated review. It admits the cells as printed; their use in a fire attack is decided by the step 17 Fire review. The original registry, the first supplement, and their verified subjects are unchanged.
