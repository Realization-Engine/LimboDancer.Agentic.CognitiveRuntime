...
### 19.8 Fail-closed acceptance-readiness assessment (partial ASL-OT-03.4)

`TirCuratedAcceptanceReadinessEvaluator` is a C# diagnostic contract (`ContractVersion: 1.0.0`), **not** a persisted acceptance record or authorization API. It first reproduces the exact history bundle; a forged projection or stale evidence fails rather than generating a plausible result. The result pins the history payload digest, subject, declared use, and deterministic blocker codes. The evaluator reports whether review remains open; whether an exact structural report reproduces from the captured source and passes all applicable structural gates; whether each source fragment has a verified source attestation with matching registry/source hashes and visual dependency references and no disputed attestation; and whether the captured bundle still carries blocking findings. The source comparison remains an operator attestation, not cryptographic proof of access to an authoritative edition. A disposition on captured evidence alone cannot silently waive a finding for a new curated use.

The later `TirCuratedSemanticAcceptanceGate` provides a separate accepting path for a typed, exact-use semantic artifact. It replays that diagnostic source gate, binds the exact proposal and declared use, requires every subject source fragment to be covered by the artifact's cited rules, detects conflicting exact predicates, and requires an independent affirmative domain vote. A reject vote requires an independent adjudicator to approve the disputed interpretation before `TirCuratedAcceptedReview` can be created. Its accepted projection leaves the captured extraction and original open history unchanged. The existing readiness evaluator without typed semantics continues to report its capability blockers; it cannot promote opaque proposal text by itself. The source reviewer and domain reviewer are still explicit identities supplied by the calling workflow, with the user-directed xUnit review serving as reviewer in this implementation.

It always reports that semantic validation, declared-use dependency closure, independent acceptance decision, and material-disagreement adjudication are unavailable. Consequently `CanAccept` is always false. These are missing capabilities, not optional booleans the caller may assert. The assessment is intentionally ephemeral and has no JSON schema or review-state transition; if an auditable acceptance record is later introduced, it needs its **own** versioned schema and must bind the exact evidence and policy. An opaque natural-language proposal cannot pass a semantic gate by attaching a source report, review attestation, or a fabricated `validated` flag. Until an actual curated semantic representation and those gates exist, an `accepted` or `superseded` bundle remains impossible.
