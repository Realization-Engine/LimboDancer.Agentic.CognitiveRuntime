...
### Retired at unit step 11

Live games now commit the forced back after a second defender is revealed, through the Map Studio's game actions and the game log ([ASL Unit Infantry OVR Design](<ASL Unit Infantry OVR Design.md>), section 8). The separate aggregate described above was therefore removed on 2026-09-26: the `LimboDancer.Domains.Asl.Execution` adapter, the pure transition, the in-memory and journal stores, the Host's `AddScenarioA1Return` registration and constraint evaluator, and their tests. The runtime references no ASL project. This review stays as the record of that work.
