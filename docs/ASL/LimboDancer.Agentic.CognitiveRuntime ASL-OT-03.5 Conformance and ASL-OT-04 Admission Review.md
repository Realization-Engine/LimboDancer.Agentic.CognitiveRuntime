...
| Candidate A1 fragment coverage | The representative verification sample has 14 fragments, all `unverified`; none has the eight normalized identifiers above. The committed TIR sample has no matching A1 artifacts. The C# generated candidate inventory records ten exact fragments for the eight rules and preserves the page-49 correction and B23.922 continuation. The PDF comparison evidence covers those fragments, footnote 3 and the Breach illustration. | Structural spans and initial full-text/visual comparisons are captured. Independent source-verification records and declared-use dependency closure remain pending. |
...
3. Have an independent source verifier review the comparison evidence against the supplied version-3.01 PDF (including footnote 3 and the Breach image), check the A4.14/A4.15 page-49 locator and B23.922 page crossing, and record an exact `SourceVerificationRecord` per relevant TIR fragment, with verifier identity, method, edition, and disposition. Preserve the observed layout differences; do not repair converted text in place. The PDF's older `3.0` introductory credit and user-reported ZIP designation remain separate provenance facts.
