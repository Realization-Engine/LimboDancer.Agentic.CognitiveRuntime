...
`asl-scenario-a1.second-defender-reveal-case-matrix.json` reviews the next A12.15/A4.15 branch as a candidate only. It distinguishes a second revealed SMC from a revealed MMC and leaves unknown identity, unrevealed units, and incomplete capability nondefinitive. Its xUnit review pins three previously verified source fragments and the current immutable package digest. No second-defender package is published yet.
