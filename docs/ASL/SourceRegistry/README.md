...
| `asl-scenario-a1.fire-chart-supplement.json` | Unit step 17 bounded chart supplement: the A7 Infantry Fire Table (physical page 692) and the TEM rows of the B. Terrain Chart (page 698). Pins the PDF, the `pdftotext 4.00 -table` extractor and page-text hashes, the normalization rule, both transcription hashes, and one hash per normalized row, note, or legend item. |
...
| `asl-scenario-a1.fire-branches-pdf-comparison.json` | Unit step 18 comparison of A10.63 and A17.1, A17.11, and A17.3 with their physical page text, verified by `AslScenarioA1FireSourceReview.BuildBranches`. |
| `asl-scenario-a1.fire-case-matrix.json` | Fire case matrix, from unit step 17 and revised at step 18: the user's Fire rulings of 2026-09-26, the resolution, 11 cases, and digests of the chart supplement, transcriptions, catalog 1.1.0, and 61 verified fragments. |
